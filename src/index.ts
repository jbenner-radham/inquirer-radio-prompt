/**
 * Based upon: https://github.com/SBoudrias/Inquirer.js/blob/9d0c1408c1b2e425e3967d934ac22203d970fcce/packages/checkbox/src/index.ts
 */
import {
  createPrompt,
  useState,
...
function check<Value>(item: Item<Value>): Item<Value> {
  return isSelectable(item) ? { ...item, checked: true } : item;
}

function uncheck<Value>(item: Item<Value>): Item<Value> {
  return isSelectable(item) ? { ...item, checked: false } : item;
}
...
export default createPrompt(
  <Value>(config: CheckboxConfig<Value>, done: (value: Value | undefined) => void) => {
    ...
    const choices = normalizeChoices(config.choices).map(choice =>
      choice instanceof Separator || !config.default || config.default !== choice.value
        ? choice
        : { ...choice, checked: true }
    );
    const [items, setItems] = useState<ReadonlyArray<Item<Value>>>(choices);

    const numberOfChoicesChecked = choices
      .filter(choice => !(choice instanceof Separator) && choice?.checked).length;

    if (numberOfChoicesChecked > 1) {
      throw new ValidationError(
        '[radio prompt] Multiple choices are selected. Only up to one may be selected.'
      );
    }
    ...
    const bounds = useMemo(() => {
      ...
      if (first === -1) {
        throw new ValidationError(
          '[radio prompt] No selectable choices. All choices are disabled.'
        );
      }
    ...
    if (
      ...
      if (
        items.length > pageSize && (theme.helpMode === 'always' || (theme.helpMode === 'auto' && firstRender.current))
      ) {
